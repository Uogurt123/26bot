...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ *{context.user_data['name']}* — чудово!\n\nВведи своє *прізвище*:",
        parse_mode="Markdown"
    )
...
async def get_surname(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🏫 Введи свій *клас* _(наприклад: 9А, 10Б)_:",
        parse_mode="Markdown"
    )
...
async def teacher_password_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text.strip() == TEACHER_PASSWORD:
        user_id = str(update.effective_user.id)
        data = load_data()
        data["users"][user_id] = {
        ...
        save_data(data)
        await update.message.reply_text(
            f"✅ *Вітаємо, {context.user_data.get('name', '')}!*\n\n👨‍🏫 Роль: Вчитель\n\nДоступ надано! 🎉",
            parse_mode="Markdown"
        )
        await show_teacher_menu(update, context)
        return ConversationHandler.END
    else:
        keyboard = [[InlineKeyboardButton("🔙 Назад", callback_data="back_to_student")]]
        await update.message.reply_text(
            "❌ *Невірний пароль!*\n\nСпробуй ще раз:",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="Markdown"
